import { toGec } from './countries';

// src/lib/factbook.js
//
// Parses CIA World Factbook country entries from the factbook.json mirror.
//
// This lives outside the route handler so that other routes can call it directly.
// The trade route used to fetch `/api/factbook` over HTTP from inside the server,
// which cost an extra round trip and broke whenever the externally-visible origin
// differed from one the server could reach.
...
/**
 * Fetch and parse one country's CIA World Factbook entry.
 *
 * Returns `{ data }` on success or `{ error, status }` on failure. Callers that
 * need an HTTP response wrap it; callers that just need the data (the trade route)
 * call this directly instead of making a round trip through their own server.
 */
export async function getFactbookData(isoCode) {
  const gecCode = toGec(isoCode);
  const region = regionMapping[gecCode];
  
  if (!gecCode || !region) {
    return { error: `Country ${isoCode} is not covered by the CIA World Factbook dataset`, status: 404 };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { error: 'The Factbook source did not respond successfully', status: response.status };
    }
    ...
    // Some Factbook fields carry inline HTML (`<p>`, `<b>`, `<br>`). The UI renders
    // these as plain text, so the markup has to come out here or it shows up
    ...
    return { data: factbookData };
    
  } catch (error) {
    console.error('Factbook API error:', error);
    return { error: 'Could not read the Factbook entry for this country', status: 500 };
  }
} 